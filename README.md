...
本固件支持通过IO扩展板控制小狗舵机和LED灯光，可以通过语音命令控制小狗做出各种动作。

### IO扩展板配置
- IO扩展芯片：HT8574ARSZ（I2C地址：0X40），连接到ESP32S3的GPIO48(SDA)和GPIO38(SCL)
- 支持的功能：
  - 4个舵机控制：前左舵机(P1)、前右舵机(P5)、后左舵机(P0)、后右舵机(P4)
  - 2个流水灯控制：流水灯1(P2)、流水灯2(P3)

### 语音控制指令
通过以下语音指令可以控制小狗的动作和灯光效果：

**基本动作指令**
- "小狗坐下" - 让小狗坐下（前腿抬起，后腿放下）
- "小狗站起来" - 让小狗站立（所有腿都抬起）
- "小狗走路" - 让小狗行走（对角线腿交替抬起放下）
- "小狗跑起来" - 让小狗奔跑（前后腿交替抬起放下，速度更快）
- "小狗摇头" - 让小狗摇头（前腿交替抬起放下）
- "小狗停下" - 停止小狗的动作
